...
// **A password with a percent sign in it is still a password.** `%ra` is not a
// valid escape, so decoding the fragment threw `URIError` out of an effect and
// took the whole activity page with it — a blank screen for a link an organizer
...
check(/Zapisz się na tę aktywność/.test(await body()),
    "and it is the enrollment form rather than an empty error boundary");
check(await evaluate(`
    const field = document.querySelector("input[type=password]");
    return field ? field.value : "";
...
// Back with the real one, and it still works: accepting and pressing enroll puts
// them in.
await go(`${APP}/activities/PROG-1-LB#PROG1-LA`, `document.body.innerText.length > 0`);
await wait(800);
...
await wait(3500);
check(!/Zapisz się na tę aktywność/.test(await body()),
    "the password that came in the link is the one that enrolls them");
