...
import { InstanceTheme, ThemeColors } from "./api/CoreApi";
import { theme as testIds } from "./theme";
import { FALLBACK_STACK, typography } from "./typography";

/**
 * An installation's own colors and typeface, turned into a Mantine theme.
 *
 ...
 *
 * Four brand colors, each generating the ten shades Mantine wants — which is
 ...
 * so an operator states four colors and the ladder follows.
 *
 ...
 * cannot. The status colors — `red`, `orange`, `teal`, `green`, `yellow`,
...
type Colors = Record<string, MantineColorsTuple>;
...
function brandOverride(branding: InstanceTheme | undefined): MantineThemeOverride {
    ...
    // one that was stated rather than inventing a second color.
    const primary = brand(colors, "primary", light.primary, dark.primary, MANTINE_PRIMARY);

    // **The shade the operator's own color landed on.** A ramp is generated
    // *around* a color, so the value they typed is rarely index 6 — and index 6
    ...
    // two colors land in different places.
    const primaryShade = {
    ...
    // A `Divider` with no color of its own draws the instance's second brand
    // color. It is a rule and carries no text, so it is somewhere a second
    // color can land without a contrast question attached.
...
/** Where in its own generated ramp a color sits. Mantine's default otherwise. */
...
/**
 * Registers one brand color as a pair of ramps and a virtual color over them.
 *
 * A virtual color is Mantine's own way of saying "this name means that ramp in
 ...
 * stepped toward when the theme has named nothing better.
 */
function scheme(colors: ThemeColors | undefined, contrast: string): Record<string, string> {
    const set: Record<string, string> = {};
    if (!colors) return set;

    // **The theme's own text, where it has one.** Stepping a surface toward
    // plain black turns a white panel into a plain gray one, which is what the
    // first photographs of these themes showed: an installation whose brand is
    // blue got gray rows. Text is the color that has to be legible on that
    // surface anyway, so moving toward it both separates the row and carries
    // the hue.
    const toward = colors.text ?? contrast;
    ...
    put("--aj-page-bg", colors.body);
    put("--mantine-color-body", colors.surface);
    put("--mantine-color-default", colors.surface);
    put("--mantine-color-text", colors.text);
    put("--mantine-color-dimmed", colors.dimmed);
    put("--mantine-color-default-border", colors.border);
    put("--mantine-color-anchor", colors.link);

    if (colors.surface) {
        set["--mantine-color-default-hover"] = mix(colors.surface, 94, toward);
        ...
        // did not reach them left those two screens gray while everything around
        ...
        // and 88 percent stand where `gray-0`, `gray-1` and `gray-2` stood
        // against white.
        set["--aj-row"] = mix(colors.surface, 96, toward);
        set["--aj-row-hover"] = mix(colors.surface, 92, toward);
        set["--aj-row-active"] = mix(colors.surface, 88, toward);
    }

    // The rows' own text, for the same reason: gray on somebody else's panel is
    // a contrast this side cannot answer for.
    put("--aj-text", colors.text);
    put("--aj-dimmed", colors.dimmed);
    ...
    put("--aj-nav-bg", colors.navBackground);
    put("--aj-nav-text", colors.navText);
    put("--aj-nav-active-bg", colors.navActiveBackground);
    put("--aj-nav-active-text", colors.navActiveText);
    put("--aj-header-bg", colors.headerBackground);
    put("--aj-header-text", colors.headerText);
    ...
    put("--aj-header-border", colors.border);

    if (colors.navBackground) {
        // Darker than the ground it sits on, as `blue-7` was against `blue-6`.
        set["--aj-nav-hover"] = mix(colors.navBackground, 88, "#000000");
    }
    ...
    // what the colored bars it is borrowed from do — and nothing that carries
    ...
    put("--aj-nav-accent", colors.accent);
    // And the second brand color, on rules. It draws no text either.
    put("--aj-secondary", colors.secondary);

    // The signed-out shell's foot is the navigation's bar in another place, so
    // it is the navigation's colors rather than a second pair of keys.
    put("--aj-footer-bg", colors.navBackground);
    put("--aj-footer-text", colors.navText);
    put("--aj-footer-border", colors.navBackground);

    if (colors.navBackground && colors.navText) {
        ...
        // the navigation's own blue, so all of them are the text color let down
        // toward the ground it is on.
        set["--aj-nav-muted"] = mix(colors.navText, 72, colors.navBackground);
        set["--aj-nav-divider"] = mix(colors.navText, 40, colors.navBackground);
        set["--aj-nav-scrollbar"] = mix(colors.navText, 55, colors.navBackground);
    }

    if (colors.navBackground) {
        set["--aj-footer-hover"] = mix(colors.navBackground, 88, "#000000");
    }

    if (colors.navBackground && colors.navText) {
        // The documents at the foot were the *page's* quiet color, which is
        // unreadable the moment the foot is a saturated bar.
        set["--aj-footer-muted"] = mix(colors.navText, 72, colors.navBackground);
    }
...
/**
 * Whether a color is dark enough that what sits on it should be drawn for a
 * dark ground.
 *
 * An instance colors its bars, so the foot of a page can be a saturated blue in
 * the **light** scheme — and a mark drawn for paper is then dark ink on dark
 * blue. The scheme cannot answer this; only the color underneath can.
 */
export function isDarkGround(color: string): boolean {
 ...
 * inspector open — gets a color it can compare.
 *
 * Both operands passed the Server's six-hexadecimal-digits rule, so there is
 * nothing here that can compose a value which is not a color.
 */
function mix(color: string, percent: number, toward: string): string {
    const channel = (at: number) => {
        const a = parseInt(color.slice(at, at + 2), 16);
        const b = parseInt(toward.slice(at, at + 2), 16);
