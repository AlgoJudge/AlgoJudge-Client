...
/**
 * Splits a paragraph into text and `$…$` maths. `\$` is a literal dollar.
 * An unclosed `$` is an error rather than a hint to guess where it ended.
 */
export const splitInline = (source: string, blockIndex?: number): InlineSegment[] => {
    ...
    for (let i = 0; i < source.length; i++) {
        ...
        if (mathStart === -1) {
            ...
            assertLatexSubset(buffer, blockIndex);
            segments.push({ kind: "math", text: buffer });
            buffer = "";
            mathStart = -1;
        }
    }
    ...
    if (mathStart !== -1) {
        throw new ContentError("Niedomknięty wzór — brakuje zamykającego $", blockIndex);
    }
    if (buffer) segments.push({ kind: "text", text: buffer });
    return segments;
};
