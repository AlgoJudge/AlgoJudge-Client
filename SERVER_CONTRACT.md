...
| 12.3 | **`ProblemDetail.limits` is declared and unfillable.** The Server cannot populate it without reading the opaque config, which the guardrail forbids; the decision says the Client derives limits from the chain. The field is dead on the wire | Withdrawing a contract field, or moving work into the Client |
