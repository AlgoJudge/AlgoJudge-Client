...
| 12.11 | **Three documents describe reversed code.** `AlgoJudge-Server/CLAUDE.md` on `EvaluationJob`, `int` keys and `Result` doubling as the job record; ~~`AlgoJudge-Client/CLAUDE.md` and `DECISIONS_AND_OPEN_QUESTIONS.md:1510` on "seven" opaque values against the specification's five~~. ~~**The Server half stands**~~ **Closed — 2026-08-30, later the same day.** Every one of the three was corrected in this pass: `AlgoJudge-Server/CLAUDE.md` now strikes the deferral and records that `EvaluationJob` is a Server entity with `Result` referencing it, lists the model as "activities, problems, submissions", and says there is exactly one `int` key left. This entry outlived the defect it names by about an hour | Proposals prepared; no status promoted |
